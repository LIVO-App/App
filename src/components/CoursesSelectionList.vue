...
const changeEnrollment = async () => {
    ...
    const groups = Object.keys(remaining_courses[selected_context.value][selected_area.value]);

    let count = 0;
    let tmp_course: CourseCardElements | undefined = undefined;
    let course: CourseCardElements,available_courses: boolean,available_area_credits: boolean,available_context_credits;
    
    while (tmp_course == undefined && count < groups.length) {
        tmp_course = courses.cards[groups[count]].find(c => c.id == "" + learning_block_id);
        count++;
    }
        ...
        if (tmp_course != undefined) {
            ...
            if (unscribe
                ...
                || available_courses && available_context_credits) {
                    await executeLink($axios,undefined,
                        (response : any) => {
                            ...
                            if (!wasPending && !isPending) {
                                remaining_courses[selected_context.value][selected_area.value][course.group] += unscribe ? 1 : -1;
...
const setAlertAndOpen = (type: AvailableModal) => {
    switch (type) {
        case "max_credits":
            alert_information.message = getCurrentElement(store,'maximum_credits_error')
            break;
        case "max_courses":
            alert_information.message = getCurrentElement(store,"maximum_courses_error");
            break;
    }
...
const showCourses = (context = selected_context.value, area = selected_area.value) => {
    courses.cards = {};
    courses.order = [];
...
const remaining_courses: {
    [key: string]: { // Learning context
        [key: string]: { // Learning Area
...
if ($axios != undefined) {
    ...
    if (learning_block != undefined) {
        learning_contexts = await executeLink($axios,"/v1/learning_contexts?student_id=" + user.id + "&block_id=" + learning_block_id,
            response => {
        ...
        tmp_courses = await executeLink($axios,"/v2/courses?student_id=" + user.id + "&block_id=" + learning_block_id + "&token=" + user.token,
            response => response.data.data,
            () => []);
        courses_ids = tmp_courses.map((a: CourseSummaryProps) => a.id);
        
        await executeLink($axios,"/v1/learning_contexts/correspondence?student_id=" + user.id + "&block_id=" + learning_block_id,
            response => {
                ...
                for (const correspondence of course_correspondences) {
                    ...
                    if (course_props != undefined && learning_context_index != -1) {
                        ...
                        if (tmp_learning_area != undefined) {
                            ...
                            if (remaining_courses[tmp_learning_context.id][tmp_learning_area.id][tmp_course.group] == undefined) {
                                remaining_courses[tmp_learning_context.id][tmp_learning_area.id][tmp_course.group] = store.state.courses_per_group;
                            }
                            ...
                            tmp_card = tmp_course.toCard(
                                ...
                                open_enrollment ? "/v1/students/" + user.id + "/" + (tmp_course.pending !== "false" ? "unscribe" : "inscribe") + "?course_id=" + tmp_course.id + "&block_id=" + learning_block_id + "&context_id=" + tmp_learning_context.id : undefined,
                                undefined,
                                open_enrollment);
                            remaining_courses[tmp_learning_context.id][tmp_learning_area.id][tmp_course.group] -= tmp_card.enrollment.enrollment ? 1 : 0;
                            all_courses[tmp_learning_context.id][tmp_learning_area.id].push(tmp_card);
