<template>
    ...
    <div class="ion-padding">
        <div class="ion-padding-bottom">
            <ion-input type="text" v-model="italian_title" :label="getCurrentElement(store,'title')" :aria-label="getCurrentElement(store,'title')" color="primary" style="color: var(--ion-color-primary);" fill="outline" class="ion-margin-vertical"></ion-input><!--Da sistemare: mettere italiano e inglese-->
            <ion-input type="text" v-model="italian_message" :label="getCurrentElement(store,'message')" :aria-label="getCurrentElement(store,'message')" color="primary" style="color: var(--ion-color-primary);" fill="outline" class="ion-margin-vertical"></ion-input><!--Da sistemare: mettere italiano e inglese-->
            <ion-label color="primary">{{ getCurrentElement(store,"sections") }}:</ion-label>
            <ion-list class="ion-margin-top">
                <ion-item v-for="(selection,index) in sections_selections" :key="index + '_section'">
                    <ion-checkbox v-model="selection.value" :aria-label="sections[index].id" class="ion-padding-start" label-placement="start">{{ sections[index].id }}</ion-checkbox>
                </ion-item>
            </ion-list>
        </div>
...
import { IonHeader, IonToolbar, IonGrid, IonRow, IonCol, IonList, IonItem, IonLabel, IonInput, IonCheckbox, IonButton } from "@ionic/vue";
...
const sections_selections: Ref<{value: boolean}[]> = ref(new Array(props.sections.length).fill({value: false}));
