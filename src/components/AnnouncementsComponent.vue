<template>
    <div class="ion-padding-horizontal">
        ...
        ></ion-alert>
        <ion-modal id="publish_announcement" :is-open="publishment_open" @didDismiss="closeModal('publish_announcement')" class="fit">
            <suspense>
                <template #default>
                    <announcements-publisher :sections="sections" :current_section_index="sections.findIndex(a => a.id == selected_section)" @signal_event="setupModalAndOpen(store)" @close="closeModal('publish_announcement')" />
                </template>
                <template #fallback>
                    <loading-component />
                </template>
        ...
        <suspense>
          <template #default>
            <list-card :key="trigger" :cards_list="messages" :emptiness_message="getCurrentElement(store,'no_messages')" @signal_event="setupModalAndOpen(store)" />
          </template>
          <template #fallback>
            <loading-component />
          </template>
...
import { AnnouncementSummary, AnnouncementSummaryProps, CustomElement, OrderedCardsList } from "@/types";
...
type availableModal = "announcement" | "publish_announcement" | "empty_titles_or_messages" | "no_selected_sections" | "publish";
